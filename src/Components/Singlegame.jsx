...
export default function Singlegame({user1, user2, current, winby, board, time, id, winpo}){
    ...
    return(
        <div className="singleBoardLayout" style={{marginBottom: "25px", marginLeft: "10px"}}>
            ...
            {(winby!=="" || current===opponent) && <p className="gameView"><Link target="_blank" to={"/games/currgame/?id="+id+"&user1="+user1+"&user2="+user2+"&winpo="+winpo+"&user="+user} style={{textDecoration: "none"}}><span className="mainLoginText">View game</span></Link></p>}
            {(winby==="" && current===user) && <p className="gameView"><Link target="_blank" to={"/games/currgame/?id="+id+"&user1="+user1+"&user2="+user2+"&winpo="+winpo+"&user="+user} style={{textDecoration: "none"}}><span className="mainLoginText">Play!</span></Link></p>}
        </div>    
    )
}