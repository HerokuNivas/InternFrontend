...
export default function Singlegame({user1, user2, current, winby, board, time, id, winpo}){
    ...
    return(
        <div className="singleBoardLayout" style={{marginBottom: "25px", marginLeft: "10px"}}>
            ...
            {(winby!=="" || current===opponent) && <p className="gameView"><Link target="_blank" to={"/games/currgame/?id="+id} style={{textDecoration: "none"}} state={{user1Is: user1, user2Is: user2, currentIs: current, winbyIs: winby, boardIsIs: board, id: id, winpo: winpo}}><span className="mainLoginText">View game</span></Link></p>}
            {(winby==="" && current===user) && <p className="gameView"><Link target="_blank" to={"/games/currgame/?id="+id} style={{textDecoration: "none"}} state={{user1Is: user1, user2Is: user2, currentIs: current, winbyIs: winby, boardIsIs: board, id: id, winpo: winpo}}><span className="mainLoginText">Play!</span></Link></p>}
        </div>    
    )
}